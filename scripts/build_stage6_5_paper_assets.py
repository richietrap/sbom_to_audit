#!/usr/bin/env python3
"""Build candidate paper assets from the Stage 6.5 performance-remediation run."""
...
def _axis_endpoint_rows(rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for axis in AXES:
        ...
        if not axis_rows:
            raise ValueError(f"Stage 6.5 summary has no rows for axis {axis}")
        first = axis_rows[0]
        last = axis_rows[-1]
        output.append(
            {
...
def _write_scaling_figure(path: Path, rows: list[dict[str, str]]) -> None:
    ...
    elements = [
        ...
        'font-size="20">Stage 6.5 remediated latency across registered scale axes</text>',
        '<text x="500" y="54" text-anchor="middle" font-family="sans-serif" '
        'font-size="11">Candidate environment-specific measurements; '
        "raw worker RSS retained separately</text>",
    ]
    for axis in AXES:
        axis_rows = sorted(
            (row for row in rows if row["axis"] == axis),
...
def build(results: Path, destination: Path) -> dict[str, Path]:
    report_path = results / "stage6_5_performance_report.json"
    summary_path = results / "stage6_5_scale_summary.csv"
    memory_path = results / "stage6_5_memory_observations.csv"
    report = json.loads(report_path.read_text(encoding="utf-8"))
    if report.get("profile") != "full":
        raise ValueError("paper assets require a full Stage 6.5 performance-remediation run")
    if report.get("evaluation_status") != "CANDIDATE_NOT_FROZEN":
        raise ValueError("Stage 6.5 paper assets require candidate-only results")
    if report.get("manuscript_eligible") is not False:
        raise ValueError("Stage 6.5 results must not be manuscript-eligible")
    ...
    detailed_output = tables / "stage6_5_scale_summary.csv"
    endpoint_output = tables / "stage6_5_axis_endpoints.csv"
    figure_output = figures / "stage6_5_latency_scaling.svg"
    ...
    manifest = {
        ...
        "source_results": {
            ...
            memory_path.relative_to(ROOT).as_posix(): sha256_file(memory_path),
        },
        ...
        "interpretation_boundary": (
            "Stage 6.5 performance-remediation assets describe one environment-specific candidate "
            "run with preserved raw worker-level peak-RSS observations. They remain ineligible "
            "until targeted independent re-audit and final evaluation freeze."
        ),
    }
    manifest_path = write_json(data / "stage6_5_asset_manifest.json", manifest)
    return {
...
def main() -> int:
    ...
    print("Stage 6.5 performance-remediation candidate paper assets generated:")
