#!/usr/bin/env python3
"""Validate the Stage 6.2 Colab checkpoint execution contract."""
...
def validate_notebook(path: Path) -> list[str]:
    ...
    source = "\n".join(code_sources)
    required = {
        'STAGE = "6.2"': "Stage identifier",
        'PACKAGE_VERSION = "0.6.2"': "package version",
        'CHECKPOINT_ID = "STAGE6-2-CHECKPOINT-001"': "checkpoint identifier",
        're.fullmatch(r"[0-9a-fA-F]{40}", REF)': "exact commit validation",
        '["git", "checkout", "--detach", REF]': "detached exact checkout",
        'VENV = Path("/content/sbom_to_audit_stage62_venv")': "isolated environment path",
        ...
        '"scripts/run_stage6_2_robustness.py"': "Stage 6.2 runner",
        '"scripts/validate_stage6_2_evaluation.py"': "Stage 6.2 validator",
        '"scripts/build_stage6_2_paper_assets.py"': "paper-asset builder",
        ...
        "stage62_colab_checkpoint_evidence.zip": "checkpoint archive",
